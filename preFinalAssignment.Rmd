...
I expect that the maximum temperature, the evaporation and the average wind speed will get higher over time. The precipitation will get lower due to the loss of water (maybe this will be different at the coast). So I expect to see a remarkable change in the climate. The changes in the interior should be bigger than at the coast because of a stronger warming of the soil. The forecast will be even worse and the changes in the climatic parameters will be stronger in a shorter time.

## Data and some results:

## Timeseries data of four stations in Texas:

There are as an example the timeseries plots for maximum temperature, evaporation and precipitation for the weather station "Mccook" in the south west of Texas.
In the next plot are the locations of the 4 different observed stations marked.
...
## Data from all stations in Texas from different years for every January and August:

First plot is the head of the temperature dataset of all station for the January of 2015, followed by the same datasets for evaporation, wind and precipitation.

After that there is the plot with all stations for the different categories for the January of 2015 (with doubles).

The other plot is the same with all stations from January of 1945. In 1945 there was only one station which collected evaporation values and wind values are only collected since 1980
...
## What am I planning to do with the data/which methods I use:

I will do a timeseries analysis with the four timeseries of the stations and compare the results to see if there are differences between the coast and the interior. To see what happens in the next years I will compute forecasts for the different climate parameters.
For the datasets of all stations in Texas of the different years I will compute variograms first and then do kriging to get an overview over the data and then compare the results. I could also take a look on the spatial correlation of the different climate parameters.
